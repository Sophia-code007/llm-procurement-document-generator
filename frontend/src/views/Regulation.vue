<template>
  <div class="app-container">
    ...
    <div class="main">
      <div class="breadcrumb"><a href="/dashboard">首页</a><span>/</span><a href="#">法规库对接</a></div>
      <div class="page-header">
        <div>
          <div class="page-title">法规库对接</div>
          <div class="page-desc">查询和引用采购相关法律法规</div>
        </div>
      </div>

      <div class="search-bar">
        <input class="form-input" v-model="searchKeyword" placeholder="搜索法规名称或关键词">
        <button class="btn btn-primary" @click="searchRegulation">搜索</button>
      </div>

      <div class="card">
        <div class="card-title">常用法规</div>
        <div class="clause-item" v-for="item in regulations" :key="item.id">
          <div class="ci-header">
...
const currentPage = ref('regulation')

const regulations = ref([
  { id: 1, title: '中华人民共和国政府采购法', type: '法律', content: '第一条 为了规范政府采购行为，提高政府采购资金的使用效益，维护国家利益和社会公共利益，保护政府采购当事人的合法权益，促进廉政建设，制定本法。...' },
  { id: 2, title: '中华人民共和国招标投标法', type: '法律', content: '第一条 为了规范招标投标活动，保护国家利益、社会公共利益和招标投标活动当事人的合法权益，提高经济效益，保证项目质量，制定本法。...' },
  { id: 3, title: '政府采购货物和服务招标投标管理办法', type: '部门规章', content: '第一条 为了规范政府采购当事人的采购行为，加强对政府采购货物和服务招标投标活动的监督管理，维护国家利益、社会公共利益和政府采购当事人的合法权益，依据《中华人民共和国政府采购法》等有关法律、行政法规，制定本办法。...' },
  { id: 4, title: '招标投标法实施条例', type: '行政法规', content: '第一条 根据《中华人民共和国招标投标法》（以下简称招标投标法），制定本条例。...' }
])
...
.form-input { height: 36px; border: 1px solid #E5E6EB; border-radius: 4px; padding: 0 12px; font-size: 13px; outline: none; }
.form-input:focus { border-color: #165DFF; }

.clause-item { background: #fff; border: 1px solid #E5E6EB; border-radius: 8px; padding: 16px; margin-bottom: 12px; }
.clause-item .ci-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px; }
.clause-item .ci-title { font-size: 14px; font-weight: 600; color: #1D2129; }
.clause-item .ci-content { font-size: 12px; color: #4E5969; line-height: 1.6; padding: 8px 12px; background: #F7F8FA; border-radius: 6px; }
